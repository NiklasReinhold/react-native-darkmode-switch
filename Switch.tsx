...
const Switch: React.FC<SwitchProps> = ({ value, handlePadding = 2, borderWidth = 4 }) => {
  ...
  useEffect(() => {
    animation.value = withTiming(value ? 1 : 0, { duration: 1000 });
  }, [value]);


  const getBorderPath = useCallback((
    height: number,
    width: number,
    borderWidth: number
  ): [string, number] => {
  ...
  const background = useCallback(() => {
    ...
    return (
      <Svg viewBox={`0 0 ${dimensions.width} ${dimensions.height}`}>
        <AnimatedPath
          d={path}
          //Use animatedProps only on ios because of android bug
          //https://github.com/software-mansion/react-native-svg/issues/1845
          // -> Work around does not work
          animatedProps={Device.osName === "iOS" ? animatedFill : {}}
          fill={value ? "#82cbff" : "#3d4147"}
  ...
  const stars = useCallback(() => {
    const animatedProps = useAnimatedProps(() => {
      return {
        opacity: value
          ? interpolate(animation.value, [0, 0.4, 0.5, 1], [1, 1, 0, 0])
  ...
  const moon = useCallback(() => {
    return (
      <Svg
        viewBox={`0 0 ${handleDiameter - borderWidth * 2} ${handleDiameter - borderWidth * 2
          }`}
      >
        <AnimatedCircle
          cx="20.6%"
  ...
  }, [handleDiameter, borderWidth, value]);
  ...
  }, [dimensions, handleDiameter, borderWidth, value]);
    ...
    return (
      <Animated.View style={[styles.cloud, animatedStyle]}>
        <Svg
          ...
          viewBox={`${-cloudStrokeWidth / 2} ${-cloudStrokeWidth / 2} ${cloudSvgWidth + cloudStrokeWidth
